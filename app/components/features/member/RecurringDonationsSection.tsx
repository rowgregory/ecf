...
export function RecurringDonationsSection({
  ...
  nextBillingDate?: string | any
}) {
  const reduceMotion = useReducedMotion()

  // Active subscription's next charge — shown once at the section level,
  // not on individual payment rows (a past payment has no "next" date).
  const hasActive = recurring.some((d) => d.status !== 'cancelled')
  ...
  return (
    ...
    >
      {recurring.length > 0 ? (
        <>
          {hasActive && nextBillingDate && (
            <p className="font-mono text-[10px] tracking-wide text-text-light/70 dark:text-text-dark/65 mb-2">
                ...
                <div className="flex items-start xs:items-center justify-between gap-3 flex-wrap mb-3">
                  <div>
                    ...
                    <p className="font-mono text-[10px] tracking-wide text-text-light/70 dark:text-text-dark/65 mt-0.5">
                      {d.paidAt ? `Paid ${formatDate(d.paidAt)}` : 'Payment pending'}
                    </p>
                  </div>
